...
- 允许您在搜索词中使用语法来获取所需的结果。 例如，键入 "**汽车银 2-门**"，以在搜索结果中包含搜索词中任何字的匹配项。 键入**car + 白银 +2-门**，仅查找包含全部三个词的匹配项。 键入 **"&#124;汽车&#124;银 2-门**" 以获取包含**汽车**、**银色**或**2 门**或全部三个单词的结果。 有关可在搜索查询中使用的语法的详细信息，请参阅[Azure 认知搜索中的简单查询语法](https://docs.microsoft.com/rest/api/searchservice/simple-query-syntax-in-azure-search)。<!--note from editor: The "More information:" pattern is good when there aren't any additional modifiers, which you have here. -->
  
  > [!NOTE]
  > 相关性搜索配置为要求与查询中的任何条件（而不是所有）匹配，这可能会产生与预期不同的结果。 当查询中包含布尔运算符时，尤其如此。<!--note from editor: Via style guide, it's always capital "Boolean," and it's "might" instead of "may.-->
