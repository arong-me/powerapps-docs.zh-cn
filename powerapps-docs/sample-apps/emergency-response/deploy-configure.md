...
### <a name="option-a-publish-using-the-template-app-from-appsource-preferred-option"></a>选项 A：使用 AppSource 中的模板应用进行发布（首选选项）
...
> [!IMPORTANT]
> 使用这种方法发布 Power BI 仪表板，比使用 .pbit 文件选项发布更简单。 在发布时，建议客户使用此选项（而不是使用 .pbit 文件选项）。 
