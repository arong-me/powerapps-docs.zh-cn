...
1.  在应用详细信息页面中：
    ...
    3. 如果需要，请更新应用的“说明”或“显示名称”   。

    4. 如果需要，请更新“从菜单隐藏应用”值，设置应用是否应在应用列表中显示  。 由于“紧急响应应用”是一个容器应用，因此该值默认设置为“否”   。

    5. 如果需要，请更新“应用显示排名”值，设置应用在应用列表中的显示位置  。
...
1.  选择“紧急响应应用”  记录，将其打开以进行编辑。
    ...
    2.  在“已启用设备共享”字段中选择“是”或“否”，以指定“注销”选项在移动应用中是否可用     。 如果选择“是”，将可以使用“注销”   选项。
...
可以使用 Azure Application Insights 来收集移动应用（画布应用）的详细遥测，以获取有关应用使用情况的见解。 有关此内容的详细信息，请参阅[使用 Application Insights 分析应用遥测](https://docs.microsoft.com/powerapps/maker/canvas-apps/application-insights)
