---
title: " 翻转组件 |Microsoft Docs"
description: 使用角度 JS 实现翻转组件
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340658"
---
# <a name="implementing-flip-component"></a>实现翻转组件

此示例演示如何使用第三方库在 PowerApps 组件框架中创建组件。  翻转示例组件是基于角、角度-ui、角度动画、区分角度的、引导实现的。 此代码可能无法显示所述的第三方库的最佳实践。

> [!div class="mx-imgBorder"]
> ![角度反转](../media/angular-flip.png "角度反转")
...
此示例提供了有关如何为第三方库添加依赖项的示例，展示如何在 PowerApps 组件框架、组件模型和第三方内部数据模型之间双向进行数据绑定。

"翻转组件" 示例由标签和按钮组成。 单击该按钮时，标签中的文本会切换。

- 加载组件时，标签将基于 "绑定" 属性值显示文本。 @No__t_0 包含关联的元数据。
- 对于 TwoOptions 字段，`context.parameters.[property_name].Options` 将同时包含 true 和 false 值选项。 
- 单击 "切换" 按钮，标签将使用**notifyOutputEvents**方法更新值， [getOutputs](../reference/control/getoutputs.md)方法将异步调用，并将流到 PowerApps 组件框架。 
- ClientAPI 更新绑定属性值，更新的值将流向组件标签。 你还可以使用 `ClientAPI` 将属性值更新为触发控件的[updateView](../reference/control/updateview.md)方法。 然后，该组件会更新第三方模型并更新标签。
