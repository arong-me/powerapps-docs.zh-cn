...
ms.locfileid: "72340175"
---
# <a name="implementing-increment-component"></a>实现递增组件

此示例组件演示如何将数据与 PowerApps 组件框架和错误处理绑定在一起。 此组件呈现为一个文本框，其中包含运行时中的 "`Increment`" 按钮。 文本框显示当前值，"`Increment`" 按钮是可单击的。 每次单击按钮时，文本框中的值将增加1。 可以将增量值更改为所需的任何数字。

若要实现此组件，需要先定义[清单](../manifest-schema-reference/manifest.md)文件，然后在 TypeScript 中实现自定义逻辑。

> [!div class="mx-imgBorder"]
> ![递增组件](../media/increment-control.png "递增组件")
...
单击该按钮时，文本框中的值将增加1。 更新的值将通过 `notifyOutputChanged` 方法传递到 PowerApps 组件框架。

> [!NOTE]
> 将组件配置为窗体上的字段时，可以更改增量值。

编辑文本框中的值，如果是有效的整数，则会将值更新到 PowerApps 组件框架。 您可以连续单击 "`Increment`" 按钮并进行更新。 如果该整数无效，则会弹出一条错误消息。
