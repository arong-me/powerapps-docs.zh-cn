...
ms.locfileid: "72340497"
---
# <a name="implementing-data-set-component"></a>实现数据集组件

此示例组件演示如何更改与 dataset 交互的用户体验。 例如，你只会在实体主页上以表的形式显示主页网格。 您可以生成可以根据您的选择显示数据的代码组件。 此示例将记录显示为磁贴，而不是常规表格网格。

> [!div class="mx-imgBorder"]
> ![数据集网格组件](../media/data-set-grid.png "数据集网格组件")
...
在此示例中，我们在组件清单文件中使用数据集标记定义了输入参数。 这是绑定到组件的输入属性。  
 
此组件有两个重要的容器，这些容器添加到主 div 上，并添加到传递到组件的 div 上。  第一个容器保存从视图中显示记录数据的磁贴，第二个容器用于显示某些记录何时需要更多可容纳在一页中的记录的 `Load More button`。 
 
每次调用[updateView](../reference/control/updateview.md)方法时都会生成并刷新两个容器。 对于第一个容器，我们基于列中的信息和记录数生成磁贴。 这可确保显示每条记录的磁贴及其信息。  
 
如果存在记录的以下页面，则会显示 "加载更多" 按钮，即，第二个容器可见，如果结果集中没有更多的页，则会被隐藏。  
 
在单击 "加载更多" 按钮时，将加载下一页记录，并将其追加到现有的结果集，而隐藏或显示按钮的逻辑将与前面的代码中所示的相同。 这由绑定到按钮的***onLoadMoreButtonClick***方法来处理。
 
***ToggleLoadMoreButtonWhenNeeded***函数采用输入作为数据集，检查数据集是否具有下一页，以及按钮是隐藏还是可见，并分别隐藏或显示按钮。  
 
***OnRowClick***函数使用其 GUID 值附加记录的上下文，并调用 `NavigationAPI` 的[openForm](../reference/navigation/openform.md)方法，以打开相应的记录。 此方法绑定到作为***createGridBody***方法的一部分生成的每个图块。
 
***GetSortedColumnsOnView***方法基于视图上定义的顺序返回列列表。
