...
ms.locfileid: "72340083"
---
# <a name="implementing-table-component"></a>实现表组件

此示例组件将呈现一个具有两列的表。 左栏显示 API 方法或属性的名称，右侧列显示该 API 返回的值。 可以在不同类型的设备上打开此组件，也可以修改语言或用户设置，以查看表中的值是否正确调整。

> [!div class="mx-imgBorder"]
> ![表组件](../media/table-control.png "表组件")
...
此示例提供了有关如何使用 `IClient, IUserSettings, IUtility, IFormatting interfaces` 中的方法的示例。

此组件还展示了两个实用函数，`setFullScreen` 和 `lookupObjects`。 通过单击呈现为代码组件一部分的按钮可调用这些函数。 "@No__t_0" 按钮将组件置于和全屏模式下切换。 "@No__t_0" 按钮将打开 "查找" 对话框，然后将所选记录作为文本插入 div。

在此示例中，我们将呈现一个 HTML 按钮，并将 JavaScript `onClick` 事件处理程序 `onLookupObjectsButtonClick` 附加到该按钮。 单击此按钮时，将调用 `context.utils.lookupObjects()` 方法，并作为参数传递实体名称的数组。 

此方法返回 JavaScript 承诺对象，该对象表示对查找对话框的调用的完成或失败。 如果成功解决了承诺，则用户选择的查找对象将作为参数传递给回调方法，并可作为 data.id、data.name、数据 entityType 引用。

回调方法将此信息作为 HTML 注入到在代码组件上呈现的 div，以向用户显示所选结果。 如果承诺被拒绝，则会调用错误回调方法，以便组件可以相应地处理错误情况。
