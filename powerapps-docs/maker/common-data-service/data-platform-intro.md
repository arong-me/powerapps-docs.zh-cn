...
description: Common Data Service、实体和服务器端逻辑简介。
author: clwesene
manager: kfile
...
## <a name="dynamics-365-and-the-common-data-service"></a>Dynamics 365 与 Common Data Service
...
* **可跨 Dynamics 365 和 PowerApps 重复使用的技能** &ndash; 具有 PowerApps 或 Dynamics 365 中以前的技能的用户现在可以跨新的 Common Data Service 平台利用这些技能。 创建实体、窗体、图表等现在在各个应用程序中是公用的。
    ...
    > [!NOTE]
    > Dynamics 365 for Finance and Operations 当前需要数据集成器的配置来让 Finance and Operations 的业务数据在 Common Data Service 内可用。
...
在开发应用程序时，可以使用标准实体、自定义实体或混合使用。 Common Data Service 默认提供标准实体。 这些内容根据最佳实践进行了设计，以获取组织内最常见的概念和情形。
