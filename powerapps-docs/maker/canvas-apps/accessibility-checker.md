...
| 检查屏幕项的顺序| 提示      | 当 TabIndex 属性大于 1 时。 通过将 TabIndex 属性设置为数字值（如 1、2、3 和 4），应用创建者可以设置自定义选项卡顺序。 此提示会提醒查看此屏幕的交互顺序。 最佳做法是执行 TabIndex 属性为 0 的设计。  | 确保屏幕元素顺序与你所希望的通过 Tab 键逐个浏览的顺序相同。 | 当屏幕阅读器读取应用的元素时，它们应按用户将看到的顺序（而不是直观性欠佳的顺序）显示。  |
