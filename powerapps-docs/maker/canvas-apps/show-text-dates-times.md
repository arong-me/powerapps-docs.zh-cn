...
1. 添加一个名为“ShowText” **[的“](controls/control-text-box.md)标签**”控件，然后将“**Text[”](controls/properties-core.md)** 属性设置为以下公式：
   <br>**Now()**
...
2. 将 **ShowText[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
...
3. 添加名为 **BirthDate[ 的](controls/control-text-input.md)** 文本输入控件，将其移至 **ShowText** 下。

4. 在 **BirthDate** 中键入你的出生月和日（例如 **05/18**）。

5. 将 **ShowText[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
...
1. 添加名为 **ArrivalDateTime[ 的](controls/control-text-input.md)** 文本输入控件，然后按以下格式键入日期和时间：
   <br>**5/10/85 6:15 AM**
2. 添加一个名为“ShowDate” **[的“](controls/control-text-box.md)标签**”控件，然后将“**Text[”](controls/properties-core.md)** 属性设置为以下公式：
...
3. 将 **ShowDate[ 的 ](controls/properties-core.md)** Text 属性更改为以下公式：
   ...
   > 
   > 
4. 若要使用多个内置格式之一，请将 **ShowDate[ 的 ](controls/properties-core.md)** Text 属性更改为以下公式：
   ...
   > 
   > 
5. 若要使用自定义格式，请将 **ShowDate[ 的 ](controls/properties-core.md)** Text 属性更改为以下公式：
...
1. 添加名为 **ArrivalDate[ 的](controls/control-text-input.md)** 文本输入控件，然后在其中键入日期（例如 **5/10/85**）。

2. 添加一个名为“FormatDate” **[的“](controls/control-text-box.md)标签**”控件，然后将“**Text[”](controls/properties-core.md)** 属性设置为以下公式：
   <br>**DateValue(ArrivalDate.Text)**
   
    **FormatDate** 显示你所键入的日期，只是年份显示为四位数。
3. 将 **FormatDate[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
   <br>**DateValue(ArrivalDate.Text, "fr")**
   
    **FormatDate** 按法国用户的习惯将日显示在月之前。
4. 若要使用多个内置格式之一，请将 **FormatDate[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
   <br>**Text(DateValue(ArrivalDate.Text), DateTimeFormat.LongDate)**
   
    **FormatDate** 显示星期几、月、日、年。
5. 若要使用自定义格式，请将 **FormatDate[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
...
1. 添加名为 **ArrivalTime[ 的](controls/control-text-input.md)** 文本输入控件，然后在其中键入 **6:15 AM**。

2. 添加一个名为“ShowTime” **[的“](controls/control-text-box.md)标签**”控件。

3. 若要使用多个内置格式之一，请将 **ShowTime[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
   <br>**Text(DateTimeValue(ArrivalTime.Text), DateTimeFormat.LongTime)**
   
    **ShowTime** 显示指定的时间，包括秒。
4. 若要使用自定义格式，请将 **ShowTime[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
...
3. 添加一个名为“DateDiff” **[的“](controls/control-text-box.md)标签**”控件，然后将“**Text[”](controls/properties-core.md)** 属性设置为以下公式：
...
4. 将 **DateDiff[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：  <br>**DateDiff(DateValue(Start.Text), DateValue(End.Text), Months)**
...
1. 添加名为 **Start[ 的](controls/control-text-input.md)** 文本输入控件，然后在其中键入 **5/10/1985**。

2. 添加一个名为“DateAdd” **[的“](controls/control-text-box.md)标签**”控件，然后将“**Text[”](controls/properties-core.md)** 属性设置为以下公式：
   <br>**DateAdd(DateValue(Start.Text), 3)**
   
    ![加三天](./media/show-text-dates-times/date-add.png)
   
    **DateAdd** 显示 **5/13/1985**，此日期晚于 **Start** 中的日期三天。
3. 将 **DateAdd[ 的 ](controls/properties-core.md)** Text 属性设置为以下公式：
   <br>**DateAdd(DateValue(Start.Text), -3)**
   
    ![减三天](./media/show-text-dates-times/date-subtract.png)
   
    **DateAdd** 显示 **5/7/1985**，此日期早于 **Start** 中的日期三天。
4. 将 **DateAdd[ 的 ](controls/properties-core.md)** Text 属性更改为以下公式：
...
2. 将 **Year[ 的 ](controls/properties-core.md)** Items 属性设置为以下公式：
   <br>**Table({Year:"2014"}, {Year:"2015"}, {Year:"2016"})**

3. 将 **Month[ 的 ](controls/properties-core.md)** Items 属性设置为以下公式：
   <br>**Table({Month:"1"}, {Month:"2"}, {Month:"3"}, {Month:"4"}, {Month:"5"}, {Month:"6"}, {Month:"7"}, {Month:"8"}, {Month:"9"}, {Month:"10"}, {Month:"11"}, {Month:"12"})**

4. 将 **Day[ 的 ](controls/properties-core.md)** Items 属性设置为以下公式：
...
2. 将 **Hour[ 的 ](controls/properties-core.md)** Items 属性设置为以下公式：
   <br>**Table({Hour:"9"}, {Hour:"10"}, {Hour:"11"}, {Hour:"12"}, {Hour:"13"}, {Hour:"14"}, {Hour:"15"}, {Hour:"16"}, {Hour:"17"})**

3. 将 **Minute[ 的 ](controls/properties-core.md)** Items 属性设置为以下公式：
...
5. 在 **Hour** 中选择 **15**，在 **Minute** 中选择 **45**。
    ...
    可以向 **Hour** 和 **Minute** 添加条目，以便用户能够在更大范围内选择小时数，以更精确的方式选择分钟数。 也可添加第三个 **[下拉](controls/control-drop-down.md)** 控件，允许用户指定秒。 如果添加第三个列表，请将“**标签[”](controls/properties-core.md)** 控件的“**Text[”](controls/control-text-box.md)** 属性设置为以下表达式：<br>**Text(Time(Value(Hour.Selected.Value), Value(Minute.Selected.Value), Value(Second.Selected.Value)), DateTimeFormat.LongTime)**
