...
1. PowerApps Mobile 中的多个媒体控件（2018 年 8 月 2 日）
    ...
    PowerApps Mobile 可在各种类型的设备上运行，其中某些设备具有特定于该平台的限制：
...
1. 重新发布应用（2018 年 8 月 2 日）
    ...
    如果几个月内都未更新应用，请重新发布该应用，以便与最新版本的 PowerApps 同步，新版本中包括性能改进和其他修补程序。
...
1. **PowerApps 通过数据生成应用时，系统不会自动配置用于排序和搜索的字段**。
