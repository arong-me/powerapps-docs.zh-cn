...
> [!NOTE]
> 目前，我们不支持 Windows 平台上适用于 Power Apps 应用的新功能。 改进的 Common Data Service 选项和来宾访问等功能在此平台上不可用。 建议在 Windows 上使用 web 播放器来利用完整的功能集。 适用于 Windows 平台的 Power Apps 应用的更新将在将来公布。
...
每个应用计划的电源应用计划允许各个用户在单个门户上运行2个应用程序，用于基于 Power Apps 的全部功能的特定业务方案。 此计划为用户提供了一种简单的方法，使用户可以在更广泛的采用范围之前开始使用平台。

管理员向环境分配每个应用计划的电源应用计划后，在该环境中的应用共享时，它们将分配给未经授权的用户。 可在[此处](https://docs.microsoft.com/power-platform/admin/capacity-add-on)了解管理员如何分配每个应用计划。
...
>
> 对于应用程序的每个应用计划分配功能，仅适用于环境中的应用程序，这些应用程序是在[Power Platform 管理中心](https://admin.powerplatform.microsoft.com)中分配的每个应用计划。  
