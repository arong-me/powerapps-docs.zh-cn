...
description: 使用 Power Apps 构建全球范围内使用的应用。
author: gregli-msft
manager: kvivek
...
ms.locfileid: "74676028"
---
# <a name="build-global-support-into-canvas-apps"></a>为画布应用构建全球支持
Power Apps 是一种全球产品。 你可以在许多不同的语言和区域中构建和使用画布应用。

在构建和运行应用程序时，电源应用显示的文本已翻译为多种语言。  你将看到以本地语言显示的菜单项、对话框、功能区选项卡和其他文本。  键入和显示日期与数字时也会针对你的特定语言和区域而调整。  例如，世界上的某些地区使用 **。** （圆点或句点）作为小数分隔符，而其他使用 **，** 则为（逗号）。  
...
* 信号记录：**罗盘标题**、**位置。Latitude**， **app.activescreen**，。
...
| 作者的语言小数分隔符 | Power Apps 小数点分隔符 | Power Apps 列表分隔符 | Power Apps 链接运算符 |
| --- | --- | --- | --- |
| **.** （圆点或句点） |**.** （圆点或句点） |**,** （逗号） |**;** （分号） |
| **,** （逗号） |**,** （逗号） |**;** （分号） |**;;** （双分号） |

Power Apps 列表分隔符的更改与 Excel 列表分隔符发生的更改一致。  它影响以下项：
...
![Power Apps 公式（如果左圆括号 slider1.value 点值大于12点59）逗号通知左括号 "有效！" 逗号成功右括号分号导航左括号 "NextScreen" 逗号无右括号逗号通知左括号 "无效，请重试" 逗号错误右括号右括号](media/global-apps/operators-dot.png)

现在，在用逗号作为小数点分隔符的语言和区域中查看此相同公式，如法国或西班牙：

![Power Apps 公式（如果左圆括号 slider1.value 点值大于12逗号59分号通知左括号 "有效！"） 分号成功右括号双分号导航左括号 "NextScreen" 分号无右括号分号通知左括号 "无效，请重试" 分号错误右括号右括号](media/global-apps/operators-comma.png)
...
如果使用了 Excel，所有这些函数均已组合到单个 **Value** 函数中。  由于 Power Apps 为日期/时间值和数字提供不同的类型，因此它们在此处进行了分解。
