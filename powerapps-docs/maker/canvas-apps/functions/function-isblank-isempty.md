...
`Coalesce( value1, value2 )` 是 `If( Not IsBlank( value1 ), value1, Not IsBlank( value2 ), value2 )` 的更简洁等效项，且不需要**value1**和**value2**进行两次计算。  如果在此处没有 "else" 公式，则[ **if**函数](function-if.md)返回*空白*。
...
| **合并（""）** | 测试第一个参数为空字符串。 由于第一个参数为空字符串，并且没有更多的参数，因此该函数返回*空白*。   |*blank* |
