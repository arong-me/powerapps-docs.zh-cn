...
这些数据类型对它们的长度没有预设的限制。 在浏览器中或在设备上的基础 JavaScript 实现可能会施加限制，但它通常为超过 100 MB。 但是，在设备上的可用内存量可能会施加另一个很可能低于 100 MB 的限制。 若要确定您的应用程序将运行这些限制范围内，测试应运行的所有设备上的常见方案。
