...
> [!div class="mx-imgBorder"]
> ![显示在具有 "所有者" 字段的库控件中的帐户](media/working-with-references/accounts-displayowner.png)
...
为 **[@Teams]** 和 **[@Users]** 使用[全局歧义消除运算符](functions/operators.md#disambiguation-operator)，以确保使用的是全局实体类型。 在这种情况下，您不需要它，但最好的方法是进行构建。 一对多关系通常在库的记录范围内发生冲突，这种做法可以避免这种混乱。
...
> [!div class="mx-imgBorder"]
> ![用于启用公式级别错误管理的试验开关](media/working-with-references/accounts-iferror.png)
...
> [!div class="mx-imgBorder"]
> ![添加了项目组上方的组合框控件，其 Items 属性设置为 Users](media/working-with-references/filter-insert-combobox.png)
...
1. 最后，将**库**控件的**Items**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > ![显示所有记录或特定用户或团队的已筛选库](media/working-with-references/filter-combobox.png)

进行这些更改后，可以显示所有记录，也可以基于用户或团队筛选它们：

> [!div class="mx-imgBorder"]
> ![基于单选控件和组合框显示不同筛选结果的动画](media/working-with-references/filter-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![使用 IsType 按所有者类型进行筛选](media/working-with-references/filter-bytype.png)
...
1. 在省略号菜单上，选择 "**复制这些项**"。
    ...
    > [!div class="mx-imgBorder"]
    > ![使用树视图复制多个控件](media/working-with-references/patch-copy.png)

1. 在同一菜单上，选择 "**粘贴**"。

    > [!div class="mx-imgBorder"]
    > ![使用树视图粘贴多个控件](media/working-with-references/patch-paste.png)
...
1. 选择已复制的**收音机**控件，然后更改以下属性：
    ...
    - 项`[ "Users", "Teams" ]`
    - 缺省值`If( IsType( Gallery1.Selected.Owner, Users ), "Users", "Teams" )`

    > [!div class="mx-imgBorder"]
    > ![已从单选控件中删除所有选择](media/working-with-references/patch-noall.png) 
...
1. 为团队选择可见的**组合框**控件，然后将其**DefaultSelectedItems**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > !["团队" 组合框的默认属性集](media/working-with-references/patch-default-teams.png)
...
1. 将该按钮的**OnSelect**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > ![按钮控件上的公式设置](media/working-with-references/patch-button.png)

复制的**单选按钮**和**组合框**控件显示库中当前选定帐户的所有者。 使用相同的控件，可以通过选择按钮将帐户的所有者设置为任何团队或用户：

> [!div class="mx-imgBorder"]
> ![显示具有用户或团队的所有者修补程序的动画](media/working-with-references/patch-allthree.gif)
...
1. 在屏幕右侧附近的 "**属性**" 选项卡上，打开 "**数据源**" 列表，然后选择 "**帐户**"。
    ...
    > [!div class="mx-imgBorder"]
    > ![显示具有空白值的其他字段的窗体控件](media/working-with-references/form-insert.png)  

1. 将窗体的**Item**属性设置`Gallery1.Selected`为。

    > [!div class="mx-imgBorder"]
    > ![显示从库中的选定项填充的其他字段的窗体控件](media/working-with-references/form-item.png)
...
1. 在 "**字段**" 窗格中，选择省略号，然后选择 "**添加自定义卡片**"。
    ...
    > [!div class="mx-imgBorder"]
    > ![用于添加自定义卡片的命令](media/working-with-references/form-customcard.png)
...
1. 将 "**标签**" 控件插入自定义卡，然后将标签的 " **Text** " 属性设置为在库中使用的公式：
    ...
    > [!div class="mx-imgBorder"]
    > ![显示标签控件中的 "所有者" 字段的自定义卡片](media/working-with-references/form-displayowner.png)
...
> [!div class="mx-imgBorder"]
> ![将公司名称字段显示为不需要的客户数据类型的联系人实体](media/working-with-references/customer-companyname.png)
...
> [!div class="mx-imgBorder"]
> !["数据" 窗格中的帐户、团队、用户和联系人实体](media/working-with-references/customer-datasources.png)

"**客户**" 和 "**所有者**" 字段的处理与此类似，你可以按字面 > 复制**应用（"** **另存为**"，然后指定其他名称）并进行以下简单的替换：
...
| 筛选广播的**Items**属性 | **[&nbsp;"All"、&nbsp;"Users"、&nbsp;"团队"&nbsp;]** | **[&nbsp;"所有"、&nbsp;"帐户"、&nbsp;"联系人"&nbsp;]** |
...
> [!div class="mx-imgBorder"]
> ![应用了简单更改的所有者应用派生的客户应用](media/working-with-references/customer-simple-update.png)

**客户**和**所有者**之间的两个重要区别要求更新库中的公式和窗体：

1. 当你按名称引用这些实体类型时，**帐户**与**联系人**之间的一对多关系优先。 **\[使用\@** " **帐户\@" \[** 而不是 "联系人"，而不是使用 "联系人"。 通过使用[全局歧义消除运算符](functions/operators.md#disambiguation-operator)，可以确保引用**IsType**和**AsType**中的实体类型。 此问题仅存在于库和窗体控件的记录上下文中。
...
> [!div class="mx-imgBorder"]
> ![更新库中副标题标签控件的 "文本" 属性](media/working-with-references/customer-update.png)
...
> [!div class="mx-imgBorder"]
> ![显示帐户、团队、用户、联系人和传真实体的数据窗格](media/working-with-references/faxes-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![关于查找的副标题控件的已更新文本属性](media/working-with-references/regarding-label.png)
...
> [!div class="mx-imgBorder"]
> ![显示有关多对一关系的 "传真" 实体的关系](media/working-with-references/activity-manytoone.png)

如果显示 "**帐户**" 实体的关系，则将显示可以是 "**相关**查找" 字段的源的所有实体。

> [!div class="mx-imgBorder"]
> ![显示关于一对多关系的 Account 实体的关系](media/working-with-references/activity-onetomany.png)
...
1. 在屏幕右侧附近的 "**属性**" 选项卡上，将库的**项目**设置为 "**帐户**"。
    ...
    > [!div class="mx-imgBorder"]
    > ![在属性窗格中将项目设置为帐户](media/working-with-references/activitypointer-accounts.png)

1. 将库的 "布局" 设置为 "**标题**"，然后将 "标题" 字段设置为 "**帐户名**"。

    > [!div class="mx-imgBorder"]
    > ![在 "属性" 窗格中将 "库" 控件的 "布局" 设置为标题](media/working-with-references/activitypointer-account-name.png)
...
> [!div class="mx-imgBorder"]
> ![显示传真列表的帐户库中所选内容的动画](media/working-with-references/activitypointer-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![显示活动实体的实体列表](media/working-with-references/activitypointer-entity.png)

**活动**实体是特殊的。 每次向 "**传真**" 实体中添加一条记录时，系统还会在 "**活动**" 实体中创建一条记录，其中包含所有活动实体的公用字段。 在这些字段中，"**主题**" 是最有趣的一项。

通过只更改上一示例中的一行，可以显示所有活动。 替换`Gallery2.Selected.Faxes`为`Gallery2.Selected.Activities`。
...
> [!div class="mx-imgBorder"]
> ![显示 IsType 函数所需的所有实体的 "数据" 窗格](media/working-with-references/activity-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![将 text 属性设置为公式以显示有关传真、电话呼叫和其他活动的信息](media/working-with-references/activitypointer-type.png)
...
> [!div class="mx-imgBorder"]
> ![带有电话呼叫详细信息的扩展文本属性](media/working-with-references/activitypointer-phonecall.png)

因此，该应用程序会显示活动的完整列表。 对于所有类型的活动都会显示 "**使用者**" 字段，而无论公式是否将其考虑。 对于你知道的活动类型，可以显示有关每个活动的类型名称和特定类型的信息。

> [!div class="mx-imgBorder"]
> ![显示不同类型的活动的信息的完成屏幕](media/working-with-references/activitypointer-complete.png)
...
> [!div class="mx-imgBorder"]
> ![在创建实体时启用附件和注释](media/working-with-references/notes-entity.png)

如果选中 "启用附件" 复选框，则将创建与 "**注释**" 实体的**相关**关系，如下图所示为 "**帐户**" 实体：

> [!div class="mx-imgBorder"]
> ![显示通过一对多关系与说明的关系的帐户实体](media/working-with-references/notes-relationships.png)
