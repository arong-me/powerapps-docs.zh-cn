...
1. 在左侧导航栏中，选择 "**数据** >  个**实体**"。
1. 在实体列表中，选择 "**帐户**"。
1. 在右上角，打开 "筛选器" 列表（默认设置为 "默认**值**"），然后选择 "**全部**"。
1. 向下滚动，直到出现 "**所有者**" 字段。

 > [!div class="mx-imgBorder"]
 > 帐户实体 @ no__t 上的 @no__t 0Owner 字段
...
> [!div class="mx-imgBorder"]
> "数据" 窗格中的 @no__t 0Accounts、团队和用户实体 @ no__t-1
...
> [!div class="mx-imgBorder"]
> 显示在 "所有者" 字段中的 "库" 控件中显示的 0Accounts "@ no__t-1" @no__t
...
你使用的是 **[@Teams]** 和 **[@Users]** 的[全局歧义消除运算符](functions/operators.md#disambiguation-operator)，以确保使用的是全局实体类型。 在这种情况下，您不需要它，但最好的方法是进行构建。 一对多关系通常在库的记录范围内发生冲突，这种做法可以避免这种混乱。
...
> [!div class="mx-imgBorder"]
> 0Experimental 开关，用于启用公式级别错误管理 @ no__t-1 @no__t
...
> [!div class="mx-imgBorder"]
> @no__t 项库上的0Added 组合框控件，其 Items 属性设置为 Users @ no__t-1
...
> [!div class="mx-imgBorder"]
> 基于组合框控件 @ no__t-1 中设置的值 @no__t 0Filtered 库
...
1. 最后，将**库**控件的**Items**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > 显示所有记录的0Filtered 库，或特定用户或团队 @ no__t-1 @no__t

进行这些更改后，可以显示所有记录，也可以基于用户或团队筛选它们：

> [!div class="mx-imgBorder"]
> ![Animation 根据单选控件和组合框 @ no__t 显示不同的筛选结果
...
> [!div class="mx-imgBorder"]
> 通过使用 IsType @ no__t ![Filter owner type
...
1. 在省略号菜单上，选择 "**复制这些项**"。
    ...
    > [!div class="mx-imgBorder"]
    > 使用树视图 @ no__t @no__t 0Copy 多个控件

1. 在同一菜单上，选择 "**粘贴**"。

    > [!div class="mx-imgBorder"]
    > 使用树视图 @ no__t @no__t 0Paste 多个控件

1. 将复制的控件移到库的右侧。

    > [!div class="mx-imgBorder"]
    > @no__t 0Moved 将控件复制到库 @ no__t 的右侧
...
1. 选择可见的**组合框**控件，然后将 " **DefaultSelectedItems** " 属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > 为 "用户" 组合框0Default 属性设置 @ no__t-1 @no__t
...
1. 为团队选择可见的**组合框**控件，然后将其**DefaultSelectedItems**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > 为 "团队" 组合框0Default 属性设置 @ no__t-1 @no__t
...
1. 将该按钮的**OnSelect**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > 在 Button control @ no__t-1 上 @no__t 0Formula 设置

复制的**单选按钮**和**组合框**控件显示库中当前选定帐户的所有者。 使用相同的控件，可以通过选择按钮将帐户的所有者设置为任何团队或用户：

> [!div class="mx-imgBorder"]
> @no__t 0Animation 显示所有者与用户或团队 @ no__t 的修补程序
...
1. 在屏幕右侧附近的 "**属性**" 选项卡上，打开 "**数据源**" 列表，然后选择 "**帐户**"。
    ...
    > [!div class="mx-imgBorder"]
    > 显示具有空白值 @ no__t-1 的其他字段的 @no__t 0Form 控件  

1. 将窗体的**Item**属性设置为 `Gallery1.Selected`。

    > [!div class="mx-imgBorder"]
    > ![Form 控件显示从库 @ no__t 中的选定项填充的其他字段
...
1. 在 "**字段**" 窗格中，选择省略号，然后选择 "**添加自定义卡片**"。
    ...
    > [!div class="mx-imgBorder"]
    > 用于添加自定义卡片 @ no__t 的 @no__t 0Command
...
1. 将 "**标签**" 控件插入自定义卡，然后将标签的 " **Text** " 属性设置为在库中使用的公式：
    ...
    > [!div class="mx-imgBorder"]
    > 0Custom 卡片显示标签控件中的 Owner 字段 @ no__t-1 @no__t

对于库中的每个选择，包含记录的所有者的帐户的更多字段将显示在窗体中。 如果使用 "**修补程序**" 按钮更改所有者，窗体控件也会显示该更改。

> [!div class="mx-imgBorder"]
> @no__t 0Animation 显示窗体控件，以响应库 @ no__t 中的更改
...
> [!div class="mx-imgBorder"]
> ![Contact 实体将公司名称字段显示为不需要 @ no__t 的客户数据类型
...
> [!div class="mx-imgBorder"]
> "0Accounts"、"团队"、"用户" 和 "联系人" 实体 @no__t

"**客户**" 和 "**所有者**" 字段的处理与此类似，你可以按字面复制应用（**文件** >  "**另存为**"，然后指定其他名称）并进行以下简单的替换：
...
| 筛选广播的**Items**属性 | **[@no__t "所有"，&nbsp; "Users"，@no__t "团队" &nbsp;]** | **[@no__t "All"、&nbsp; "Accounts"、@no__t "Contacts" &nbsp;]** |
...
> [!div class="mx-imgBorder"]
> 从应用了简单更改的所有者应用派生的 @no__t 0Customer 应用 @ no__t-1

**客户**和**所有者**之间的两个重要区别要求更新库中的公式和窗体：

1. 当你按名称引用这些实体类型时，**帐户**与**联系人**之间的一对多关系优先。 使用 **\[ @ no__t-3Accounts]** ，而不是使用**帐户**。使用 **\[ @ no__t-7Contacts]** ，而不是使用**联系人**。 通过使用[全局歧义消除运算符](functions/operators.md#disambiguation-operator)，可以确保引用**IsType**和**AsType**中的实体类型。 此问题仅存在于库和窗体控件的记录上下文中。
...
> [!div class="mx-imgBorder"]
> ![Update 到库 @ no__t 中副标题标签控件的 Text 属性

进行这些更改后，可以在 "**联系人**" 实体中查看和更改 "**公司名称**" 字段。

> [!div class="mx-imgBorder"]
> @no__t 0Animation，其中显示了选择联系人如何更改其他控件以及 @ no__t 的格式-1
...
> [!div class="mx-imgBorder"]
> 显示帐户、团队、用户、联系人和传真实体 @ no__t 的 @no__t 0Data 窗格
...
> [!div class="mx-imgBorder"]
> 0Updated 的子标题控件的文本属性，适用于查找 @ no__t-1 @no__t

进行这些更改后，可以像处理**所有者**和**客户**查找一样处理**相关**查找。

> [!div class="mx-imgBorder"]
> ![Animation，显示选择库中的项如何更改其他控件以及 @ no__t 的格式-1
...
> [!div class="mx-imgBorder"]
> 0Relationships-显示关于多对一关系 @ no__t-1 的传真实体的 @no__t

如果显示 "**帐户**" 实体的关系，则将显示可以是 "**相关**查找" 字段的源的所有实体。

> [!div class="mx-imgBorder"]
> 0Relationships-显示关于一对多关系 @ no__t 的帐户实体 @no__t
...
1. 添加另一个屏幕。
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Insert 为空 screen @ no__t-1
...
1. 在屏幕右侧附近的 "**属性**" 选项卡上，将库的**项目**设置为 "**帐户**"。
    ...
    > [!div class="mx-imgBorder"]
    > 在属性窗格中将项0Set 到帐户 @ no__t-1 @no__t

1. 将库的 "布局" 设置为 "**标题**"，然后将 "标题" 字段设置为 "**帐户名**"。

    > [!div class="mx-imgBorder"]
    > "属性" 窗格中的 "库" 控件的0Set 布局 @no__t-no__t-1
...
1. 将新库的**Items**属性设置为 `Gallery2.Selected.Faxes`。
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Set 显示传真 @ no__t 的库的 Items 属性

1. 将库的布局设置为**标题和副标题**，然后将 "标题" 字段设置为显示 "**使用者**" 字段（可能**为小写）** 。

    > [!div class="mx-imgBorder"]
    > ![Set title to Subject field @ no__t-1

在帐户列表中选择一项时，传真列表仅显示该帐户的传真。

> [!div class="mx-imgBorder"]
> @no__t 0Animation 显示传真列表 @ no__t 的帐户库中的选定内容
...
> [!div class="mx-imgBorder"]
> 显示 Activity 0List @ no__t-1 的实体的 @no__t

**活动**实体是特殊的。 每次向 "**传真**" 实体中添加一条记录时，系统还会在 "**活动**" 实体中创建一条记录，其中包含所有活动实体的公用字段。 在这些字段中，"**主题**" 是最有趣的一项。

通过只更改上一示例中的一行，可以显示所有活动。 将 `Gallery2.Selected.Faxes` 替换为 @no__t。

> [!div class="mx-imgBorder"]
> @no__t 第二个库的 Items 属性的0Change，从传真改为活动 @ no__t-1

记录来自**活动**实体，但你仍可以使用**IsType**函数来标识它们的活动类型。 同样，在将**IsType**与实体类型一起使用之前，必须添加数据源。

> [!div class="mx-imgBorder"]
> ![Data 窗格，显示 IsType 函数 @ no__t 所需的所有实体
...
> [!div class="mx-imgBorder"]
> 用于显示传真、电话呼叫和其他活动 @ no__t 的信息的公式 @no__t 0Set text 属性
...
> [!div class="mx-imgBorder"]
> 创建实体 @ no__t 时 @no__t 0Enabling 附件和注释-1

如果选中 "启用附件" 复选框，则将创建与 "**注释**" 实体的**相关**关系，如下图所示为 "**帐户**" 实体：

> [!div class="mx-imgBorder"]
> 0Account 实体通过一对多关系显示与说明的关系 @ no__t-1 @no__t
