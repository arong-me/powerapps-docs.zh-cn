...
1. 打开[此站点](http://web.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc)。
1. 在左侧的导航栏中，选择**数据** > **实体**。
1. 在实体列表中，选择**帐户**。
...
此查找字段可以指从一条记录**团队**实体或**用户**实体。 这些实体中的不是每个记录具有权限要**所有者**; 如果遇到问题，请选中支持的角色。
...
祝贺： 你已完成记录引用所使用的最困难的方面。 其他用例都更简单，因为它们不访问记录的字段。 作为案例，需要筛选，这将在本部分中探讨。
...
> [!div class="mx-imgBorder"]
> ![项属性设置为用户添加上述库组合框控件](media/working-with-references/filter-insert-combobox.png)
...
1. 复制并粘贴**组合框**控制，直接将副本移原始，然后设置这些属性的副本：
    ...
    应用程序将只有一个组合框显示在时，根据单选控件的状态。 因为它们是另一个的正上方，它们将显示为相同更改其内容的控件。
...
1. 上**属性**选项卡的右侧窗格中，打开**数据源**列表，并选择**帐户**:
...
治疗**客户**并**所有者**字段非常相似，你可以按原义复制应用程序 (**文件** > **另存为**，然后指定一个不同的名称) 并进行以下简单替换：
...
1. 一个对多关系之间**帐户**并**联系人**按名称引用这些实体类型时，优先考虑。 而不是**帐户**，使用 **\[\@帐户]**，而不是**联系人**，使用 **\[ \@联系人]**。 通过使用[全局消除歧义运算符](functions/operators.md#disambiguation-operator)确保指中的实体类型**IsType**并**AsType**。 仅记录上下文的库和窗体控件中存在此问题。

1. **所有者**字段必须具有一个值，但**客户**字段可以是*空白*。 若要显示的类型名称不正确的结果，测试与这种情况下[ **IsBlank**函数](functions/function-isblank-isempty.md)，并改为显示一个空的文本字符串。

这些更改的两个都在同一个公式，将显示在窗体中的自定义卡中，并将**文本**库的标签控件的属性：
...
> [!div class="mx-imgBorder"]
> ![更改在联系人选项的动画显示基于推动的其他控件和窗体中的更改的库控件](media/working-with-references/customer-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![在传真中显示所做更改的动画基于推向其他控件和窗体的更新的库控件](media/working-with-references/regarding-allthree.gif)
...
如果显示的关系**帐户**实体，则可能引发的所有实体**有关**查找字段显示：
...
1. 上**属性**选项卡的右侧窗格中，设置库的**项**到**帐户**:

    > [!div class="mx-imgBorder"]
    > ![将项目设为在属性窗格中的帐户](media/working-with-references/activitypointer-accounts.png)
...
1. 设置新的库**项**属性设置为`Gallery2.Selected.Faxes`。
    ...
    > [!div class="mx-imgBorder"]
    > ![将项属性设置为传真基于库控件](media/working-with-references/activitypointer-faxes.png)
...
如果选中此复选框，则将创建**有关**与关系**说明**实体，如图所示的**帐户**实体：
