...
1. 插入具有**空白垂直**布局的[**库**](controls/control-gallery.md)控件：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Add 空垂直库 @ no__t-1

    将显示订单详细信息的新库显示在左上角：

    > [!div class="mx-imgBorder"]
    > 订单详细信息库 @ no__t 的 @no__t 0Default 位置

1. 关闭 "**数据**" 窗格，然后将详细信息库调整到新标题栏下面的右下角：

    > [!div class="mx-imgBorder"]
    > 订单详细信息库 @ no__t 的 @no__t 0Final 位置
...
1. 将详细信息库的**Items**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > "Orders" 实体与 "订单详细信息" 实体 @ no__t 之间的0One 关系 @no__t
...
1. 调整标签的大小，以显示完整的文本：
    ...
    > [!div class="mx-imgBorder"]
    > 按订单详细信息 @ no__t @no__t 0Show 产品名称

    此表达式将从 "**订单详细信息**" 实体中的记录进行审核。 记录通过多对一关系保存到**订单产品**实体的**ThisItem**中：

    > [!div class="mx-imgBorder"]
    > "订单详细信息" 实体和订单 "Product" 实体 @ no__t 之间 ![Many 之间的关系

    提取 "**产品名称**" 字段（以及要使用的其他字段）：

    > [!div class="mx-imgBorder"]
    > 订单 Products 实体 @ no__t 中的 0Fields-1 @no__t
...
1. 在 "**插入**" 选项卡上的 "详细信息库" 中插入[**图像**](controls/control-image.md)控件：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Insert image control @ no__t-1
...
1. 调整图像和标签的大小并将其并排移动。
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Move image control @ no__t-1
...
1. 将图像的**image**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Show product image @ no__t-1
...
1. 将新标签的 " **Text** " 属性设置为以下表达式：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Show product quantity @ no__t-1 
...
1. 双击标签的文本，然后键入**Product**：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Change 将标签文本添加到 Product @ no__t-1
...
1. 双击新标签的文本，然后键入 "**数量**"：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Change 将标签文本添加到 no__t-1
...
1. 双击新标签的文本，然后键入**Unit Price**：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Change 将标签文本添加到 Unit Price @ no__t-1
...
1. 双击新标签的文本，然后键入 "**扩展**"：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Change 将标签文本添加到 Extended @ no__t-1
...
1. 减小详细信息库的高度，以便为屏幕底部的订单总数腾出空间：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Shorten 顺序-详细信息库 @ no__t-1

1. 将标题栏复制并粘贴到屏幕中间，然后将其复制到屏幕底部：

    > [!div class="mx-imgBorder"]
    > @no__t 0Copy 标题栏，并将复制移动到下边缘 @ no__t-1
...
1. 在 "**主页**" 选项卡上，将新标签的**填充**颜色设置为**LightBlue**：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Change 标签的填充到浅蓝色 @ no__t-1
...
1. 将组合框的**Items**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Set 组合框的 Items 属性 @ no__t-1
...
1. 在右侧边缘附近的 "**属性**" 选项卡中，向下滚动并关闭 "**允许多项选择**"，并确保已启用 "**允许搜索**"：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Disable 多重选择，并启用 @ no__t 搜索 @
...
1. 在显示的产品列表中，选择一个产品：
    ...
    > [!div class="mx-imgBorder"]
    > 在组合框中 @no__t 0Select 产品 @ no__t-1
...
1. 在 "**插入**" 选项卡上，选择 " **Media**@no__t 2"**映像**：
    ...
    > [!div class="mx-imgBorder"]
    > 0Default | no__t-1 的图像控件位置 @no__t
...
1. 将此控件的**默认**属性设置为 **""** ：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Set 的文本输入框 @ no__t 的 * * 默认值 * * 属性
...
1. 调整大小并将标签移动到文本输入控件的右侧，然后将标签的 " **text** " 属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Set 标签的 Text 属性 @ no__t-1
...
1. 双击 "数量" 的文本输入控件，然后键入一个数字。
    ...
    **扩展**价格标签将重新计算以显示新值：
    ...
    > [!div class="mx-imgBorder"]
    > ![Specify 某个数量，并显示扩展价格 @ no__t-1

## <a name="add-an-add-icon"></a>添加添加图标

1. 在 "**插入**" 选项卡上，选择 "**图标**@no__t"**添加**：

    > [!div class="mx-imgBorder"]
    > @no__t 0Insert 添加图标 @ no__t-1
...
1. 调整此图标的大小并将其移动到浅蓝色区域的右边缘，然后将图标的**OnSelect**属性设置为以下公式：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Set 图标的 OnSelect 属性 @ no__t-1
...
1. 按 F5，然后选择 "**添加**" 图标。
    ...
    > [!div class="mx-imgBorder"]
    > 添加订单详细信息 @ no__t 的 @no__t 0Animation
...
1. 在屏幕的中心，选择详细信息库的模板：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t 0Select 库模板 @ no__t-1

1. 在 "**插入**" 选项卡上，选择**图标** > **垃圾桶**：

    > [!div class="mx-imgBorder"]
    > ![Insert 垃圾桶图标 @ no__t-1

    "垃圾桶" 图标将显示在库模板的左上角。

    > [!div class="mx-imgBorder"]
    > @no__t-icon @ no__t 的位置0Default
...
1. 尝试添加和删除订单的各种订单详细信息：
    ...
    > [!div class="mx-imgBorder"]
    > @no__t-添加和删除订单详细信息 @ no__t-1

## <a name="in-conclusion"></a>结束

总之，你添加了另一个库来显示订单详细信息，以及在应用程序中添加和删除订单详细信息的控件。 你使用了以下元素：

- 第二个库控件，通过一对多关系链接到订单库：**Gallery2** =  @ no__t-2
- "**订单详细信息**" 实体与 "**订单产品**" 实体之间的多对一关系： `ThisItem.Product.'Product Name'` 和 `ThisItem.Product.Picture`
...
- 删除**订单详细信息**记录的**删除**函数： `Remove( 'Order Details', ThisItem )`
