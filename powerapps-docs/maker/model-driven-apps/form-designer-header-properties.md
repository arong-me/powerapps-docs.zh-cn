...
开发者可以控制模型驱动应用程序窗体页眉的密度，以便满足窗体用户的需要。
...
为了确保始终显示关键信息，此框架显示只读字段值，而用户则不能直接编辑页眉中的字段值。 也不允许使用自定义控件或 Web 资源之类可视化效果。
...
低密度窗体页眉允许用户直接编辑页眉中的字段值。 还允许使用自定义控件和 Web 资源之类可视化效果。  
...
用户在窗体页眉中选择 V 形控件时，将显示页眉弹出项目。 供用户编辑字段值，并显示窗体页眉中的可视化效果，如自定义控件或 Web 资源。
...
> [!NOTE]
> - 请使用新窗体设计器，因为经典窗体设计器不能显示或隐藏页眉弹出项目。   
...
使用新窗体设计器或经典窗体设计器编辑窗体时，可能会看到一些与窗体页眉有关的消息。 下面提供有关每条消息的详细信息和显示的原因。
