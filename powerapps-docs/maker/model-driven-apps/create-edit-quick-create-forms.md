...
虽然您可以为系统活动实体创建快速创建窗体（约会实体除外），但是它们并不支持快速创建窗体。 当前，禁用约会实体的快速创建窗体的选项不受支持。 通过在实体定义中选择**启用快速创建窗体**并为该实体创建快速创建窗体，可以启用任何其他[更新后的实体](create-design-forms.md)以及任何自定义实体以支持这些窗体。 
...
## <a name="enable-quick-create-form-property-behavior-for-activities"></a>活动的“启用快速创建窗体”属性行为
**启用快速创建窗体**属性可以为所有标准活动启用或禁用，除定期约会外。 此属性让您可以更改为大部分活动默认显示的窗体。 默认情况下，**启用快速创建窗体**属性启用，快速创建窗体是显示在应用程序区域以及支持它的活动实体中的窗体。 
...
下表指未当**启用快速创建窗体**属性在统一接口客户端中*启用*时默认显示哪些窗体。
...
<sup>1</sup>当**启用快速创建窗体**属性启用时，活动出现在全局的**创建**或 **+ 新建**按钮中。 在这种情况下，如果存在快速创建窗体则使用它，如果不存在则使用主窗体。 如果禁用**启用快速创建窗体**，实体条目将不会显示。

### <a name="classic-web-client-form-display-behavior"></a>经典 Web 客户端窗体显示行为

下表指未当**启用快速创建窗体**属性在经典 Web 客户端中*启用*时默认显示哪些窗体。
...
社交窗格是一种特殊情况，因为它不使用**启用快速创建窗体**属性，而使用其他活动实体的其他窗体，如此处所示。
...
在从版本 8.2 导入解决方案，而不管解决方案中的**启用快速创建窗体**属性的值时，以下实体将重置为默认窗体显示值，主窗体将显示：任务、电话联络、电子邮件和约会。 如此一来，您需要在导入后将这些活动实体的**启用快速创建窗体**选项重置回*启用*。
 
如果在版本 9.0 解决方案中对启用了**启用快速创建窗体**的实体进行了自定义，导入后值不会更改。  但是，如果您将**启用快速创建窗体**选项设置为对任务、电话联络、电子邮件和约会实体*禁用*，此值将会被覆盖为启用值。 如此一来，您需要在导入后将这些活动实体的**启用快速创建窗体**选项重置回禁用。 
