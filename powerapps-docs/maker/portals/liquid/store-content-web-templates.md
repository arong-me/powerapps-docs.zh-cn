...
当 Web 模板的最常用案例将会呈现 HTML 时，呈现整个响应（通过取消选择**使用网站标题和脚注**）提供给您呈现选择的任何基于文本的格式的选项。 这是 Web 模板的 **MIME 类型**属性变为相关的位置。 在不使用网站标题和脚注的页面模版呈现时，HTTP 响应 Content-Type 标题将设置为关联的 Web 模板的 MIME 类型。 （如果不提供 MIME Type，将使用text/html。）这样就可以通过各种选项使用 Liquid 呈现非 HTML 内容。 常用案例将呈现 [RSS](http://en.wikipedia.org/wiki/RSS) 源，方法是设置 MIME 类型 application/rss+xml。  
