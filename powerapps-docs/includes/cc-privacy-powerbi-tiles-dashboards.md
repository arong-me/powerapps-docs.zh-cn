通过启用 Power BI 磁贴和仪表板的嵌入功能，当用户嵌入 Power BI 磁贴或仪表板时，该用户对 Common Data Service 的 [!INCLUDE[pn_azure_active_directory](pn-azure-active-directory.md)] 授权令牌将用于通过隐式授权对 Power BI 服务进行身份验证，同时为最终用户提供无缝式“单一登录”体验。  
