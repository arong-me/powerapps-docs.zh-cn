---
title: 响应对 PowerApps 客户数据的导出数据主体权限 (DSR) 请求 | Microsoft 文档
description: 响应对 PowerApps 客户数据的导出数据主体权限 (DSR) 请求
services: powerapps
suite: powerapps
...
---
# <a name="responding-to-export-data-subject-rights-dsr-requests-for-powerapps-customer-data"></a>响应对 PowerApps 客户数据的导出数据主体权限 (DSR) 请求

“数据可移植性权限”允许数据主体以电子格式（这是一种结构化的、常用的、计算机可读和可互操作的格式）请求其个人数据副本，这些数据可能会传输给另一个数据控制者：

* 网站访问：[PowerApps 创建者门户](https://web.powerapps.com)、[PowerApps 管理中心](https://admin.powerapps.com/)和 [Office 365 服务信任门户](https://servicetrust.microsoft.com/)

* PowerShell 访问：PowerApps cmdlet（[创建者 cmdlet](https://go.microsoft.com/fwlink/?linkid=871448)、[管理员 cmdlet](https://go.microsoft.com/fwlink/?linkid=871804)）和[本地网关 cmdlet](https://go.microsoft.com/fwlink/?linkid=872238)
...
画布应用  | PowerApps 管理中心 <br> PowerApps 创建者门户 |  PowerApps cmdlet
画布应用权限  | PowerApps 管理中心 <br> PowerApps 创建者门户    | PowerApps cmdlet
网关 | PowerApps 创建者门户*** | 本地网关 cmdlet
网关权限 | PowerApps 创建者门户*** |
自定义连接器 | |    创建者：可用 <br> 管理员：开发中
自定义连接器权限 | |    创建者：可用 <br> 管理员：开发中
连接 | | 创建者：可用 <br> 管理员：开发中
连接权限  | | 创建者：可用 <br> 管理员：开发中
PowerApps 用户设置、用户应用设置和通知 | | 创建者：可用 <br> 管理员：开发中

> ** 在引入 Common Data Service for Apps 后，如果数据库是在环境中创建的，那么环境权限和模型驱动的应用权限会作为记录存储在 CDS for Apps 数据库实例中。 请参阅[针对 Common Data Service for Apps 客户数据执行 DSR 请求](https://go.microsoft.com/fwlink/?linkid=872251)，以获取有关如何响应使用 CDS for Apps 的用户的 DSR 请求的指导。

> *** 只有当资源所有者明确授予访问权限时，管理员才能从 [PowerApps](https://web.powerapps.com) 访问这些资源。  否则，管理员将需要利用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804)。

## <a name="prerequisites"></a>先决条件

### <a name="for-users"></a>对于用户
任何具有有效 PowerApps 许可证的用户都可以使用 [PowerApps](https://web.powerapps.com) 或[创建者 cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 来执行本文档所述的用户操作。

### <a name="for-admins"></a>对于管理员
若要使用 PowerApps 管理中心、Microsoft Flow 管理中心或 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来执行本文档所述的管理操作，将需要具有以下权限的帐户：
...
* 如果需要在另一个用户的资源中搜索，还将需要 [Office 365 全局管理员](https://support.office.com/article/assign-admin-roles-in-office-365-for-business-eac4d046-1afd-4f1a-85fc-8219c79e1504)或 [Azure Active Directory 全局管理员](https://docs.microsoft.com/azure/active-directory/active-directory-assign-admin-roles-azure-portal)权限，否则只能访问在其中具有环境管理员权限的环境和环境资源。
...
管理员可以通过以下步骤，导出由来自 [PowerApps 管理中心](https://admin.powerapps.com/)的特定用户创建的所有环境：
...
### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-PowerAppsEnvironment 函数来导出在 PowerApps 中有权访问的环境：
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
...
可以在环境中向用户分配权限（如“环境管理员”、“环境创建者”等），这些权限作为“角色分配”保存在 PowerApps 中。 在引入 CDS for Apps 后，如果数据库是在环境中创建的，那么这些“角色分配”会作为记录存储在 CDS for Apps 数据库实例中。 有关详细信息，请参阅[管理 PowerApps 中的环境](environments-administration.md)。

### <a name="for-environments-without-a-cds-for-apps-database"></a>对于没有 CDS for Apps 数据库的环境

#### <a name="powerapps-admin-center"></a>PowerApps 管理中心
管理员可以通过以下步骤，从 [PowerApps 管理中心](https://admin.powerapps.com/)中导出用户的环境权限：
...
5. 如果用户具有对任一角色的访问权限，请转到“用户”页，复制详细信息，然后将它们粘贴到文档编辑器，如 Microsoft Word。
...
### <a name="powerapps-maker-portal"></a>PowerApps 创建者门户
用户可以从 [PowerApps](https://web.powerapps.com) 导出应用。 有关如何导出应用的分步说明，请参阅[导出应用](environment-and-tenant-migration.md#exporting-an-app)。
...
5. 一旦有权访问用户的每个应用，可以从 [PowerApps](https://web.powerapps.com) 中导出这些应用。 有关如何导出应用的分步说明，请参阅[导出应用](environment-and-tenant-migration.md#exporting-an-app)。
...
### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-RoleAssignment 函数来导出有权访问的所有应用的应用角色分配：
...
### <a name="powerapps-admin-center"></a>PowerApps 管理中心
管理员可以通过以下步骤，导出 [PowerApps 管理中心](https://admin.powerapps.com/)中用户的应用角色分配：
...
### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-Connection 函数导出其有权访问的所有连接：
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
允许管理员使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户创建的连接的函数目前正在开发中。
 
## <a name="step-6-export-the-users-permissions-to-shared-connections"></a>步骤 6：导出共享连接的用户权限
### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-ConnectionRoleAssignment 函数来导出其有权访问的所有连接的连接角色分配：
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
允许管理员使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户的连接角色分配的函数目前正在开发中。
...
### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-Connector 函数导出已创建的所有自定义连接器：
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
允许管理员使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户创建的自定义连接器的函数目前正在开发中。

## <a name="step-8-export-the-users-permissions-to-custom-connectors"></a>步骤 8：导出自定义连接器的用户权限

### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
用户可以使用 [PowerApps 创建者 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 中的 Get-ConnectorRoleAssignment 函数来导出有权访问的自定义连接器的所有连接器角色分配：
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
允许管理员使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户的自定义连接器角色分配的函数目前正在开发中。
 
## <a name="step-9-export-powerapps-notifications-user-settings-and-user-app-settings"></a>步骤 9：导出 PowerApps 通知、用户设置和用户应用设置
PowerApps 向用户发送几种类型的通知，包括与用户共享应用以及 CDS for Apps 导出操作完成时。 用户的通知历史记录在 [PowerApps](https://web.powerapps.com) 中对其可见。

PowerApps 还存储几个不同的用户首选项和设置，用于提供 PowerApps 运行时和门户体验，包括用户上一次打开应用、固定应用的时间等。

### <a name="powerapps-maker-powershell-cmdlets"></a>PowerApps 创建者 PowerShell cmdlet
允许使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户的 PowerApps 通知、用户设置和用户应用设置的函数目前正在开发中。

### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
允许管理员使用 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 导出用户的 PowerApps 通知、用户设置和用户应用设置的函数目前正在开发中。
...
### <a name="powerapps-maker-portal"></a>PowerApps 创建者门户
用户可以通过执行以下步骤从 [PowerApps](https://web.powerapps.com) 导出存储在网关服务中的个人数据：

1. 从 [PowerApps](https://web.powerapps.com) 的租户默认环境中，选择“网关”，然后选择你有权访问的每个网关的“详细信息”。
...
PowerApps 许可证始终包含 Microsoft Flow 功能。 除了包含在 PowerApps 许可证中以外，Microsoft Flow 还作为独立服务提供。 有关如何响应使用 Microsoft Flow 服务的用户的 DSR 请求的指导，请参阅[针对 Microsoft Flow 客户数据执行 DSR](https://go.microsoft.com/fwlink/?linkid=872250)。
...
有关如何响应使用 CDS for Apps 的用户的 DSR 请求的指导，请参阅[针对 Common Data Service 客户数据执行 DSR](https://go.microsoft.com/fwlink/?linkid=872251)。
