---
title: 响应对客户数据的删除数据主体权限 (DSR) 请求 | Microsoft Docs
description: 响应对客户数据的删除数据主体权限 (DSR) 请求
services: powerapps
suite: powerapps
...
---
# <a name="responding-to-delete-data-subject-rights-dsr-requests-for-customer-data-in-powerapps"></a>在 PowerApps 中响应对客户数据的删除数据主体权限 (DSR) 请求

从组织的客户数据中删除个人数据的“删除权限”是一般数据保护条例 (GDPR) 中的关键保护。 删除个人数据包括删除系统生成的日志，但不包括审核日志信息。

PowerApps 允许用户生成业务线应用程序，此应用是组织日常操作的关键部分。 当某位用户离开组织时，你将需要手动检查并确定是否删除此用户创建的某些数据和资源。 从 Azure Active Directory 删除用户帐户时，将自动删除其他个人数据。
...
\*** 对于包含 Common Data Service (CDS) for Apps 数据库的环境，环境权限（即向用户分配“环境创建者”和“管理员”角色）作为记录存储在此数据库中。 请参阅[针对 Common Data Service 执行 DSR](https://go.microsoft.com/fwlink/?linkid=872251)，以获取有关如何响应 CDS for Apps 用户 DSR 的指导。
...
画布应用  | PowerApps 管理中心 <br> PowerApps 站点| PowerApps cmdlet
画布应用权限  | PowerApps 管理中心 | PowerApps cmdlet
连接 | | 应用创建者：可用 <br> 管理员：开发中
连接权限 | | 应用创建者：可用 <br> 管理员：开发中
自定义连接器 | | 应用创建者：可用 <br> 管理员：开发中
自定义连接器权限 | | 应用创建者：可用 <br> 管理员：开发中

\** 在引入 CDS for Apps 后，如果数据库是在环境中创建的，那么环境权限和模型驱动应用权限就会作为记录存储在该数据库实例中。 请参阅[针对 Common Data Service 执行 DSR](https://go.microsoft.com/fwlink/?linkid=872251)，以获取有关如何响应 CDS for Apps 用户 DSR 的指导。

## <a name="prerequisites"></a>先决条件

### <a name="for-users"></a>对于用户
任何具有有效 PowerApps 许可证的用户都可以使用 [PowerApps 站点](https://web.powerapps.com)或[应用创建者的 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871448) 来执行本文档所述的用户操作。

### <a name="for-administrators"></a>对于管理员
若要使用 [PowerApps 管理中心](https://admin.powerapps.com/)、Microsoft Flow 管理中心，或 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来执行本文档所述的管理员操作，将需要具有以下两个权限的帐户：

- PowerApps 计划 2 的付费或试用许可证。 可以[注册获取试用许可证](http://web.powerapps.com/trial)并在 30 天后续订。
...
管理员可以通过以下步骤，授予对由来自 [PowerApps 管理中心](https://admin.powerapps.com/)的特定用户创建的环境的管理访问权限：
...
管理员可以通过以下步骤，查看并删除由来自 [PowerApps 管理中心](https://admin.powerapps.com/)的特定用户创建的环境：
...
### <a name="delete-environments-created-by-a-user-using-powershell"></a>使用 PowerShell 删除由用户创建的环境
 管理员可以通过 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 中的 Remove-AdminEnvironment 函数 删除由用户创建的所有环境：
...
## <a name="step-2-delete-the-users-permissions-to-all-other-environments"></a>步骤 #2：删除所有其他环境的用户权限
...
4.  在“环境角色”中，分别选择“环境管理员”和“环境创建者”，并使用搜索栏搜索用户名。
...
如果管理员决定不删除用户的画布应用，他们可以通过 [PowerApps 管理员 PowerShell cdmlet](https://go.microsoft.com/fwlink/?linkid=871804) 中的 Set-AdminAppOwner 函数重新分配用户所有的应用：
...
## <a name="step-4-delete-the-users-permissions-to-canvas-apps"></a>步骤 #4：删除画布应用的用户权限
...
6.  如果用户有权访问应用，则从应用的“共享”屏幕中，删除其权限，然后选择“保存”。
...
该函数允许管理员使用 [PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来查找和删除用户的连接，目前正在开发中。

## <a name="step-6-delete-the-users-permissions-to-shared-connections"></a>步骤 #6：删除共享连接的用户权限
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
该函数允许管理员使用 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来查找和删除用户的连接角色分配，目前正在开发中。
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
该函数允许管理员使用 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来查找和删除用户的自定义连接器，目前正在开发中。

## <a name="step-8-delete-the-users-permissions-to-shared-custom-connectors"></a>步骤 #8：删除共享自定义连接器的用户权限
...
### <a name="powerapps-admin-powershell-cmdlets"></a>PowerApps 管理员 PowerShell cmdlet
该函数允许管理员使用 [PowerApps 管理员 PowerShell cmdlet](https://go.microsoft.com/fwlink/?linkid=871804) 来查找和删除用户的连接器角色分配，目前正在开发中。
...
请参阅[针对 Microsoft Flow 客户数据执行 DSR](https://go.microsoft.com/fwlink/?linkid=872250)，以获取有关如何响应使用 Microsoft Flow 服务的用户的 DSR 指导。
...
某些 PowerApps 许可证（包括 PowerApps 社区计划）使组织中的用户可以创建 CDS for Apps 实例，并在 CDS for Apps 上创建和生成应用。 PowerApps 社区计划为免费许可证，用户可在单独环境中试用 CDS for Apps。 有关每个 PowerApps 许可证随附的功能，请参阅“PowerApps 定价页”。

请参阅[针对 CDS for Apps 执行 DSR](https://go.microsoft.com/fwlink/?linkid=872251)，以获取有关如何响应使用 CDS for Apps 用户的 DSR 的指导。

> [!IMPORTANT]
> 建议管理员为 PowerApps 用户完成此步骤。

## <a name="step-11-delete-the-user-from-azure-active-directory"></a>步骤 #11：从 Azure Active Directory 中删除用户
完成上述步骤后，最后一步是按照 [Office 365 服务信任门户](https://servicetrust.microsoft.com/ViewPage/GDPRDSR)中的“Azure 数据主体请求 GDPR”文档所述的步骤，删除 Azure Active Directory 用户的帐户。
